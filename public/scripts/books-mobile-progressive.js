(() => {
  ...
  function boot(attempt = 0) {
    ...
    if (document.body.dataset.booksMobileProgressiveReady === 'true') return;
    document.body.dataset.booksMobileProgressiveReady = 'true';

    let limit = INITIAL_LIMIT;
    let frame = 0;

    const controls = document.createElement('div');
    controls.className = 'mobile-progress-controls mobile-progress-controls--books';
    controls.hidden = true;
    controls.innerHTML = `
      <p class="mobile-progress-status" aria-live="polite"></p>
      <button type="button" class="mobile-progress-more">Load more books</button>
    ...
    function sync() {
      ...
      if (status) {
        status.textContent = hasMore
          ? `${mounted.toLocaleString('en-US')} of ${eligible.length.toLocaleString('en-US')} matching books shown`
          : '';
      }
    ...
    document.addEventListener('click', (event) => {
      if (event.target?.closest?.('#clear-filters,[data-book-view],.books-view-button')) {
        window.setTimeout(resetLimit, 0);
      }
    });

    const mutationObserver = new MutationObserver((records) => {
      if (records.some((record) => record.type === 'childList' || record.attributeName === 'style' || record.attributeName === 'hidden' || record.attributeName === 'data-book-view')) {
        scheduleSync();
      }
    });
    mutationObserver.observe(grid, {
      ...
      subtree: true,
      attributes: true,
      attributeFilter: ['style', 'hidden', 'data-book-view'],
    });
