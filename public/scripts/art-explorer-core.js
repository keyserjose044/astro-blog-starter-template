...
const numberOrNull = (value) => value === '' || value == null
  ? null
...
function bootArtExplorer() {
  ...
  const controls = {
    search: $('#art-search'),
    filtersToggle: $('#art-filters-toggle'),
    filtersPanel: $('#art-filters-panel'),
    filtersCount: $('#art-filters-count'),
    artist: $('#art-artist-filter'),
    movement: $('#art-movement-filter'),
    medium: $('#art-medium-filter'),
    country: $('#art-country-filter'),
    year: $('#art-viewed-year-filter'),
    period: $('#art-period-filter'),
    sort: $('#art-sort'),
    results: $('#art-results-summary'),
    clear: $('#art-clear-filters'),
    empty: $('#art-filter-empty'),
    artistsView: $('#art-artists-view'),
    timelineView: $('#art-timeline-view'),
    mapView: $('#art-map-view'),
    artistsMetrics: $('#art-artists-metrics'),
    artistsContent: $('#art-artists-content'),
    timelineMetrics: $('#art-timeline-metrics'),
    timelineContent: $('#art-timeline-content'),
    timelineHelp: $('#art-timeline-help'),
    mapMetrics: $('#art-map-metrics'),
    mapStage: $('#art-map-stage'),
    mapStatus: $('#art-map-status'),
    mapTooltip: $('#art-map-tooltip'),
    countryPanel: $('#art-country-panel'),
    mapNote: $('#art-map-note'),
  };
  ...
  const haystack = new Map(cards.map((card) => [card, norm([
    card.dataset.title,
    card.dataset.artist,
    card.dataset.country,
    card.dataset.medium,
    card.dataset.movement,
    card.dataset.artworkLabel,
  ...
  if (controls.period) {
    ...
    [...periods.values()]
      ...
      .forEach((period) => controls.period.add(new Option(period.label, period.key)));
  }
  ...
  function mapMatch(card) {
    if (!state.selectedMapCountryId) return true;
    return String(card.dataset.artCountryIds || '').split(' ').includes(state.selectedMapCountryId);
  }
  ...
  function activeFilterCount() {
    return [controls.artist, controls.movement, controls.medium, controls.country, controls.year, controls.period]
  ...
  function updateResults() {
    ...
    if (controls.filtersCount) {
      controls.filtersCount.textContent = String(active);
      controls.filtersCount.hidden = active === 0;
    }
  ...
  function clearFilters() {
    ...
    [controls.artist, controls.movement, controls.medium, controls.country, controls.year, controls.period]
  ...
  function sortCards() {
    ...
    const ordered = [...cards].sort((a, b) => {
      ...
      if (mode.startsWith('date-')) {
        comparison = compareNullable(parseDateValue(a.dataset.dateViewed), parseDateValue(b.dataset.dateViewed), mode.endsWith('asc') ? 'asc' : 'desc');
      ...
      } else if (mode.startsWith('artwork-')) {
        comparison = compareNullable(numberOrNull(a.dataset.artworkSort), numberOrNull(b.dataset.artworkSort), mode.endsWith('asc') ? 'asc' : 'desc');
      }
  ...
  function pressView(view) {
    viewButtons.forEach((button) => button.setAttribute('aria-pressed', String(button.dataset.artView === view)));
  }

  function setCollectionView(view, persist = true) {
    const next = view === 'list' ? 'list' : 'gallery';
    state.activeView = next;
    state.collectionView = next;
    explorer.hidden = true;
    controls.artistsView.hidden = true;
    controls.timelineView.hidden = true;
    controls.mapView.hidden = true;
    grid.hidden = false;
    grid.dataset.artView = next;
    pressView(next);
    if (persist) {
      try {
        const key = matchMedia('(max-width:900px)').matches ? 'lifeloggerz-art-mobile-view' : 'lifeloggerz-art-desktop-view';
  ...
  async function showView(view) {
    ...
    grid.hidden = true;
    controls.artistsView.hidden = view !== 'artists';
  ...
  function restoreView() {
    ...
    const key = mobile ? 'lifeloggerz-art-mobile-view' : 'lifeloggerz-art-desktop-view';
    let view = mobile ? 'list' : 'gallery';
    try { view = localStorage.getItem(key) || view; } catch (_error) {}
  ...
  function viewerCards() {
    const visible = getVisibleCards();
    return visible.length ? visible : cards;
  }
  ...
  function openViewer(card) {
    ...
    const details = [
      ['Date', card.dataset.artworkLabel],
      ['Movement', card.dataset.movement],
      ['Medium', card.dataset.medium],
      ['Country / origin', card.dataset.country],
      ['Viewed', card.dataset.dateViewed],
      ['Source note', card.dataset.source],
  ...
  $('#art-viewer-random')?.addEventListener('click', () => {
    const available = viewerCards().filter((card) => card !== state.viewerCard);
    openViewer(available[Math.floor(Math.random() * available.length)] || state.viewerCard);
  });
  ...
  function say(message) {
    ...
    toastTimer = setTimeout(() => { surpriseToast.hidden = true; }, 3200);
  }

  function closeSurprise() {
    surpriseMenu.hidden = true;
    surpriseTrigger.setAttribute('aria-expanded', 'false');
  }
  ...
  async function runSurprise(action) {
    ...
    if (action === 'artwork') {
      ...
      state.last.artwork = choice;
      openViewer(choice);
      return say(`Selected: ${choice.dataset.title}`);
    }
  ...
  surpriseTrigger?.addEventListener('click', () => {
    const open = surpriseMenu.hidden;
    surpriseMenu.hidden = !open;
    surpriseTrigger.setAttribute('aria-expanded', String(open));
    if (open) $('button', surpriseMenu)?.focus();
  });
  $$('[data-surprise]', surpriseMenu).forEach((button) => button.addEventListener('click', () => {
    closeSurprise();
    surpriseTrigger.focus();
    runSurprise(button.dataset.surprise);
  }));
  document.addEventListener('click', (event) => {
    if (surprise && !surprise.contains(event.target)) closeSurprise();
  });
  ...
  window.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') {
      ...
      if (state.activeView === 'artists' || state.activeView === 'timeline' || state.activeView === 'map') setCollectionView(state.collectionView);
