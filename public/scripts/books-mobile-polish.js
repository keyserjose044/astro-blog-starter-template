...
function startBooksMobilePolish() {
  ...
  /* Preserve display casing from the structured Raindrop note. The base filter
     dataset is lower-cased for matching, so explorer views should not use it as
     presentation copy when the original author value is available. */
  cards.forEach((card) => {
  ...
  /* The old floating note tooltip has been superseded everywhere by visible List
     metadata plus the shared Quilt preview/details sheet. Removing the rendered
     tooltip also makes the legacy first-tap listener in the original page inert. */
  cards.forEach((card) => {
  ...
  const handleViewportChange = () => syncCardSemantics();
  if (typeof sheetQuery.addEventListener === 'function') sheetQuery.addEventListener('change', handleViewportChange);
  else sheetQuery.addListener(handleViewportChange);
